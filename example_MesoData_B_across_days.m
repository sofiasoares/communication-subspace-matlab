...
for iMouse = 1:numel(animalList)
            ...
            if size(data2use,1)>=[maxNeuTrgt+maxNeuSrc]
                ...
                for iAreaTarget = 1:numel(this_session_areas)
                    ...
                    if size(data2use,1)>=maxNeuTrgt
                        ...
                        %                         test_y = dimStruct.(thisMouse).(sessionField).RSPp_test.RSPp_test.Y_target;
                        %                         test_x = dimStruct.(thisMouse).(sessionField).RSPp_test.RSPp_test.X_source;
                        %                         test_B = dimStruct.(thisMouse).(sessionField).RSPp.RSPp.B;
                        %
                        %                         [loss, Yhat] = RegressPredict(test_y, test_x, test_B, 'LossMeasure', 'NSE');
                        %                         1-loss
                    end
                end
            end
    ...
    disp(['Now start testing model performance on other sessions'])
        ...
        for iAreaSource = 1:numel(thisSessionAreasSource)
            ...
            for iAreaTarget = 1:numel(thisSessionAreasTarget)
                ...
                if ~isfield(sumStruct,thisSource)
                    ...
                    sumStruct.(thisSource).(thisTarget).optDim = [];
                    sumStruct.(thisSource).(thisTarget).avgPerf = [];
                    sumStruct.(thisSource).(thisTarget).avgWithinDayPerf = [];
                ...
                if dataDay1.optDimPerf>=fit_thresh && dataDay2.optDimPerf>=fit_thresh
                    ...
                    sumStruct.(thisSource).(thisTarget).optDim = [sumStruct.(thisSource).(thisTarget).optDim [dataDay1.optDim; dataDay2.optDim]];
                    sumStruct.(thisSource).(thisTarget).avgWithinDayPerf = [sumStruct.(thisSource).(thisTarget).avgWithinDayPerf mean([dataDay1.optDimPerf dataDay2.optDimPerf])];
